...
class Oauth_Request_Validator extends Zend_Validate_Abstract {
    ...
    const MISSING_RSET = 'missing_rset';
    ...
    protected $_messageTemplates = array(
        ...
        self::WRONG_RSET => "invalid_request:Specified rset does not exists",
        ...
        self::MISSING_RSET => "invalid_request:rset not specified",
    ...
    private function isValidAuthorizeEndpointRequest(Oauth_Request_Interface $request) {
        ...
        if(!$request->getRSet()){
			...
			if (!$this->validateRSetsAndScopes($request)) {
						return FALSE;
			}
		}
	...
	private function validateRSetsAndScopes(Oauth_Request_Interface $request) {

		//$scopeMapper = new Oauth_Mapper_Scope();
		$newscopeMapper = new Resourceset_Mapper_RSetScopes();
		...
		if (!$rset = $request->getRSet()) {
			$this->_error(self::MISSING_RSET);
			return FALSE;
		}

		$scopes = explode(" ", trim($scope));
		$rsets = explode(" ", trim($rset));

		foreach (array_combine($rsets, $scopes) as $rset => $sco){
			if(!empty($rsets_arr = $newscopeMapper->findrsetscope($sco, $rset))){
				$this->_error(self::WRONG_SCOPE);
				$this->_error(self::WRONG_RSET);
				return FALSE;
			}
		}
		return TRUE;
    ...
    private function validateAuthorizationCode(Oauth_Request_Interface $request) {
		...
		if(!$rset = $request->getRSet()){
         ...
         $code_builder = new Oauth_Builder_AuthorizationCodeWRS();
       	}
