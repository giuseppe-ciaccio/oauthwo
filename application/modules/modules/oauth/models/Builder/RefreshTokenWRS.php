 ...
 * @author Antonio Pastorino <antonio.pastorino@gmail.com>
 */
class Oauth_Builder_RefreshTokenWRS{
     ...
     */
    public function create_rsets(Oauth_Model_Client $client, $rests, $scopes, Oauth_Model_ResourceOwner $resource_owner) {

        $code = $this->generateRandomNumber(20);


        $refresh_token = new Oauth_Model_RefreshTokenWRS();

        $refresh_token->setCode($code);
        $refresh_token->setClient($client);
        $refresh_token->setRSets($rests);
        $refresh_token->setScopes($scopes);
        $refresh_token->setResourceOwnerId($resource_owner->getId());

        //Saving to the DB - should we?
        $refresh_tokenMapper= new Oauth_Mapper_RefreshTokenWRS();
    ...
    public function retrieve($code) {
        $codeMapper = new Oauth_Mapper_RefreshTokenWRS();
