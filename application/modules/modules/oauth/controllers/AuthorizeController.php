...
class Oauth_AuthorizeController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        {
        	$this->_code_factory = $factoryLocator->getAuthorizationCodeFactoryWRS();
			$this->_token_factory = $factoryLocator->getTokenFactory();
        }
    }
    ...
    public function processAction() {
        ...
        if ($form->getValue('yes')) {//Resource Owner says yes
		            ...
		            $rset_yesno = $data_preproc[RSET];

		            if(empty($rset_yesno) || $rset_yesno == " " || $rset_yesno == ""){
		            	$this->processApprove($form->getValues());
		            }
		            else{
		            	$this->processRSetApprove($form->getValues());
		            }
    ...
    protected function processApprove($data) {
     ...
     /**
	     ...
	     * @param array $data
	     */
	    protected function processRSetApprove($data) {
	        ...
	        if ($data[RESPONSE_TYPE] === RESPONSE_TYPE_CODE) {
	            $code = $this->_code_factory->createWRset($requesting_client, $rsets, $scopes, $resource_owner);
	            $url = $urlHelper->authorizationCodeRedirectWRS($data[REDIRECT_URI], $code, $state);
	            //var_dump($url);

	        } else {
	            //$token = $this->_token_factory->create_rsets($resource_owner, $rsets, $scopes);
	            //$url = $urlHelper->tokenRedirect($data[REDIRECT_URI], $token, $state);
	        }
    ...
    /*$action = '/v2/oauth/authorize/process';*/
       ...
       // check if new prototype
	           $rsetparam = $this->getRequest()->getParam(RSET);
	           ...
	           $rset = explode(" ", $this->getRequest()->getParam(RSET));
				$rsets = array();
				foreach ($rsets as $r) {
					$rsets[] = $this->_helper->ModelLoader->LoadRSet($r);
				}
				...
				return $form->buildRsetDisclaimer($rsets, $client)
		  			 ->injectRequestValues($this->getRequest()->getParams());
        	}
    }
